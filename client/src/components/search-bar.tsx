...
// Define the shape of search results based on usage
...
export default function SearchBar({ 
  ...
  // Type the useQuery result to avoid 'unknown' errors
  const { data: searchResults = [] } = useQuery<SearchResultItem[]>({
  ...
  // Fix: explicitly cast Set to array or use Array.from to avoid downlevelIteration issues
                  ...
                  <div className="flex flex-wrap gap-1">
                    {categories.slice(0, 4).map((category) => (
                      <button
                        key={category}
                        type="button"
                        className={cn(badgeVariants({ variant: "secondary" }), "cursor-pointer hover:bg-slate-200 text-xs")}
