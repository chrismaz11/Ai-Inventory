...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/analyze-photos", upload.array('photos', 10), async (req, res) => {
    try {
      ...
      // Analyze photos concurrently for better performance
      const analysisPromises = files.map(async (file) => {
        ...
        try {
          ...
          return []; // Return empty array on failure to preserve partial success
        }
      });

      const results = await Promise.all(analysisPromises);
      // Flatten results
      results.forEach(items => allAnalyzedItems.push(...items));
